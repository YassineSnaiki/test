...
export default function ClosedTickets() {
  ...
  useEffect(() => {
    ...
    const fetchAssignedTickets = async () => {
      try {
        ...
        // Filter to only show tickets that are "closed"
        const closedTickets = data.data.filter(
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">Closed Tickets</h1>
  ...
  // Filter tickets to only include those with status "closed"
        ...
        <div className="border rounded-lg shadow-sm">
          <div className="p-6">
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">There are no closed tickets assigned to you.</p>
            </div>
          </div>
        </div>
      ) : (
